...
public:
	LenseFlareCompositePass(RendererFrontend &p_renderer) :
		...
		m_lensFlareDirt(Loaders::texture2D().load(Config::rendererVar().lens_flare_dirt_texture, MaterialType::MaterialType_Diffuse)),
		m_lenseFlareStarburst(Loaders::texture2D().load(Config::rendererVar().lens_flare_starburst_texture, MaterialType::MaterialType_Diffuse))
	{
